...
### 6.2 容器化部署（生产模式，给同事访问）

本地开发用 §6.1 的 `start_all.ps1`（vite dev + 本地进程）；给同事访问用容器化部署（nginx 托管 + 后端容器化）：

```bash
cd docker
docker compose build backend frontend          # 构建后端/前端镜像
docker compose up -d mysql backend frontend     # 启动三个容器
```

架构：同事浏览器 → `10.66.67.26:8080` → frontend 容器（nginx 80，托管 dist + 反代 /api /ws）→ backend 容器（uvicorn 8900，含 framework+projects 源码以调 pytest）→ mysql 容器（3306）。

访问地址:

| 模式 | 服务 | 地址 |
| --- | --- | --- |
| 开发 | 前端工作台 | http://localhost:5173 |
| 开发 | 后端接口文档 | http://localhost:8900/docs |
| 生产 | 前端工作台（同事访问） | http://10.66.67.26:8080 |
| 生产 | 后端接口文档（经 nginx 反代） | http://10.66.67.26:8080/docs |

后端通过 `APP_DATABASE_URL` 读取数据库连接（容器内指向 `mysql:3306`）；`.env.example` 提供本地 Docker MySQL 默认值。项目与用例仍以文件扫描为事实来源，扫描结果同步到数据库；执行记录与报告快照落库，服务重启后页面历史仍可查询。MySQL 数据目录挂载到 `data/mysql/`，逻辑备份写入 `data/backups/`，离线镜像导出到 `data/docker-images/`（含平台镜像 `automation-platform-backend/frontend_latest.tar`，详见 `docs/部署与运维.md` §5.4）。
