"""WebUI login test for jmsco (哥伦比亚 JMS system).
...
jmsco = JMS + CO (哥伦比亚 JMS 系统). See README.md for naming convention.
"""
...
async def test_login(web_client: WebClient, base_url: str, case: dict[str, Any]) -> None:
    """数据驱动登录: 打开浏览器 -> 切换中文 -> 输入凭据 -> 人工验证码 -> 检测成功."""
    login_page = LoginPage(web_client, base_url=base_url)
    captcha = SlidePuzzleCaptcha(web_client)
    ...
    with step("切换中文界面"):
        ...
        await _attach_page_state(web_client.page, "02-切换中文后")
    ...
    with step(f"输入账号密码并提交 (case: {case['id']})"):
        ...
        await _attach_page_state(web_client.page, "03-提交登录后")
    ...
    with step("人工处理滑块验证码"):
        ...
        await _attach_page_state(web_client.page, "04-验证码处理后")
        assert solved, f"未检测到登录成功标志，登录失败 (case: {case['id']})"  # noqa: RUF001

    with step("登录成功 - 截图留存"):
        await login_page.screenshot()
        await _attach_page_state(web_client.page, "05-登录成功")
        allure.attach(
